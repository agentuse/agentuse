...
- **The release model lineup now includes Claude Fable 5.1, Gemini 3.8 Flash, and Qwen3.8 Max.** Their version aliases follow the new releases while preserving the lower-cost and previous-generation workload tiers that remain current. The Qwen curator recognizes OpenRouter's short release-date suffix, so a dated current model cannot silently fall behind an older clean ID.

- **Providers can now ship as plugins.** The plugin API supports provider adapters, conditional activation, provider-specific model discovery and readiness checks, session-aware execution, and isolated delegated runs. Packages and runtime extensions are configured separately, project-local packages can be linked in place, and provider models are available throughout dashboard creation and revision flows.
- **New `agentuse plugins` command namespace.** `plugins install <source>`, `plugins update [name]`, `plugins remove <name>`, and `plugins list` manage installed plugin packages from the CLI, globally or per project with `-l`. A local directory path is linked in place in either scope, so it does not need to be a Git repository and edits are live on the next start; append `@<full commit>` to clone and pin a local checkout instead. Shortlisted provider plugins install from their reviewed commit, so a moved release tag can never change the code that runs. Failures print one actionable line instead of a stack trace.
- **Codex, Claude Code, Pi, and portable Agent Skills integrations now build directly from AgentUse's canonical builtin skills.** One deterministic build produces self-contained local marketplaces, an npm-ready Pi package, and stable release archives without checking generated copies into sibling repositories. The Codex and Claude Code release ZIPs include marketplace metadata and can be installed after extraction without a source checkout. The release gate verifies archive structure, version coupling, bundled checksums, and byte-identical guidance before GitHub Release creation attaches all four packages.
- **Any dashboard page can be copied as a shareable link with `⌘/Ctrl+Shift+C`.** The copied URL preserves the current route and shows a confirmation toast.
- **AgentUse for Mac now checks for updates on a recurring schedule.** Checks continue while the menu-bar app runs without requiring Settings to stay open.

### Changed

- **Custom provider transports declare their wire protocol explicitly.** A `protocol: "anthropic" | "openai"` field on a `custom` transport tells AgentUse which protocol-specific behavior to apply; the old `providerOptionsKey` shortcut still works but is deprecated. Cancelling a run now aborts the transport's request signal and closes its stream instead of letting it run on, and long-running `models()` discovery is re-asked every five minutes so a serve daemon sees new models without a restart.
- **`agentuse plugins update` says "already up to date".** A source pinned to a tag or commit, or a linked package whose version has not changed, no longer reports "Updated". A project-scoped install of a plugin now shadows a global install of the same name instead of failing to load.
- **"Revise Agent" works for agents that have never run.** A revision can start from the agent file itself when there is no finished run to learn from; the page still prefers a finished run as evidence when one exists.
- **Settings tabs slide instead of swapping, and a provider row can be deep-linked.** A `?provider=` link from the Plugins tab opens Providers with that row already expanded.
- **Keyboard focus uses the app's own ring, and the macOS accent color in the desktop app.** The browser's blue focus box no longer appears around dialogs or the whole page after navigation; touch devices draw no ring, and keyboard users get a legible one.
...
- **Provider setup now installs from a release-reviewed plugin shortlist.** Each choice identifies its publisher, reviewed version, authentication requirements, and installation source. Existing subscription credentials receive the compatibility plugin and credential migration automatically when needed; new users choose a plugin explicitly. Missing-package status also recognizes already-migrated credentials so reinstalling preserves the login.
...
- **Creating and revising an agent now happens in one draft-and-refine workspace.** The authoring session remains open while the operator reviews the source, answers the author's questions, asks for changes in the same thread, and inspects the author's work before saving. Suggested ideas and existing-agent revisions open this same workspace rather than separate flows.
- **The dashboard's Schedules, Approvals, Stores, and Sessions pages have been rebuilt around their operational jobs.** Schedules use an actionable timetable with shared run-health signals; Approvals summarize pending and completed decisions; Stores explain each pipeline and its item relationships without counting intentionally dropped items as failures; and Sessions combines output search, status totals, chronological groups, an in-page reader, and reliable neighbouring-run navigation. Incomplete work is distinct from failed runs, dismissed runs can be hidden, and the whole needs-attention group can be cleared at once; rejecting a gate dismisses that run automatically.
- **A judge agent keeps one session across the attempts on a gate.** The second pre-review attempt resumes the first attempt's judge session and sends only the revised request plus which candidates changed, instead of opening a fresh context that re-reads everything and can contradict itself. The judge keeps its reasoning about what it already read, and the provider serves the long prefix from cache, so later redos cost a fraction of the first and a higher `maxRedos` on slate agents is affordable. The session is dropped whenever the gate suspends, so a human decision always gets a fresh judge; a resume is declined when the judge history grows past a fixed size or the judge file changed. The session view shows one Judge row labelled with its attempt range and the newest verdict.
- **The judge's verdict now sits where the reviewer decides.** On a manager's session page each Judge child row shows passed or failed with its per-candidate results, instead of "completed" and nothing else. The approval card shows the judge's mark and one-line reason under each option it judged, plus a footer with the judge, the attempt count, and a link to the judge session, on both the leaf's own gate and the copy that reaches the manager. The judge panel above the log now lists only the session's own verdicts, so nothing is shown twice.
- **Judge verdicts are visible above the session log.** A finished session folds its log shut, which hid the pre-review verdicts inside it. The session page now leads with a judge panel: one row per attempt with the outcome, the judge, and the per-candidate results, each linking to the marker in the log. The verify row in the log and `agentuse sessions show` list the per-candidate verdicts the same way.
- **Every delegated agent now reports its terminal result inside its own session card.** Completed and incomplete reports share one display contract across ordinary agents and judges: the collapsed card keeps the outcome headline and its descendant cards visible, while expanding it reveals the ordinary agent's report body, checks, and artifacts. Judge cards always show their full evidence, even inside a collapsed parent. Older recorded subagent results are adapted into the same card instead of rendering as a separate block below it.
- **CLI startup and the live dashboard do less repeated work.** Development builds are clearly marked in `--version` and the footer, compiled bytecode is cached instead of invoking Git on every start, the home clock no longer re-renders the page, and streamed Markdown is parsed incrementally.
...
- **Dashboard actions remain responsive with multiple tabs open.** Live streams share a connection budget across tabs, leaving capacity for API requests; additional views fall back to polling.
- **Agent creation and project discovery use a supported default thinking effort.** Internal jobs default to `low` instead of `minimal`, preventing GPT-5.4-mini requests from failing when no effort is selected. Explicit creator selections are preserved.
- **Providers from installed plugin packages now report their real model limits.** Context window, output cap, cost, and Anthropic-specific handling were lost for providers registered by an installed package, and a provider's portable system-prompt contribution was added again on every model fallback. Hosted transports also send the api key resolved for each request, so a rotated key takes effect without a restart.
- **A pre-review judge no longer fails a slate candidate it already passed.** On a gate that offers several drafts, the judge now returns one verdict per candidate and reports every failing candidate in one pass, so one revision can fix them all instead of one per bounce. A candidate that passed keeps its pass on the next attempt as long as its text is unchanged: the gate carries it forward and tells the judge not to re-litigate it, and when nothing changed at all the gate skips the judge call entirely. The bounce sent back to the agent names the passing candidates so it leaves them byte-identical.
- **The approval card no longer shows a judge verdict as if it were about a draft the judge never saw.** Each recorded verdict now carries a fingerprint of the text it judged. When the agent revised a draft after that verdict, its strip turns amber and reads "revised since attempt N, not re-judged", keeping the old reason as history instead of painting a stale failure red. A gate that reaches the reviewer without a judge look, because the redo budget was spent or a reviewer comment routed the revision straight back, now records a skipped marker, so the card footer, the judge panel, the session log, and `agentuse sessions show` say "not judged" and why.
- **The agent creator no longer proposes a filename that is already taken.** It sees the existing names and rejects a collision in-session, instead of the operator hitting an "agent exists" error when saving the draft.
- **Approval gates now reject malformed exact shell commands before review.** Commands in `changes[]` are syntax-checked without execution, so unterminated quotes and other deterministic shell failures return actionable preflight feedback instead of consuming human approval and a one-shot lease.
- **Optional media fallback checks no longer erase an agent's configured tools when the media service is unavailable.** The original tool set is preserved and the run can continue without that optional capability.
- **Interrupted continuations repair unresolved ordinary tool calls before replaying later turns.** AgentUse records an explicit unknown-outcome result without claiming the external effect failed, keeps still-running journal entries intact, and marks the resumed session running before resume hooks execute. This prevents provider protocol errors while warning the agent to inspect persisted state before retrying a side effect.
- **Filesystem mutations are serialized and writes replace files atomically.** Concurrent changes to one path no longer race, and batch `edits[]` are validated as independent, non-overlapping operations against one snapshot instead of depending on earlier edits in the same batch.
- **OpenCode Go receives stable routing headers for the full session.** Anthropic, Responses, compatible, and custom Go transports now keep requests for one run on the same provider route.
...
- **Mac, Web, and CLI now guide a new user through the same first-agent onboarding journey.** Setup creates and registers a project, runs a credential-free sample, confirms that an AgentUse runtime provider is ready, and prepares a structured handoff for Codex, Claude Code, Cursor, or another coding agent. Interactive provider login remains in Terminal rather than passing credentials or OAuth details through chat. After the prompt is copied, the Web and Mac experience waits for the created agent to appear and offers **Open agent**; Terminal setup provides the equivalent project and next-step instructions. Mac adds one short first-launch screen before this shared flow, with Launch at Login selected by default and an optional bundled CLI launcher, neither of which blocks continuing.
...
- **Human learning is deliberate; automatic capture is opt-in per channel.** A review comment becomes durable only when the reviewer checks **Learn from this comment**, uses `--remember`, or adds a learning manually. Unchecked comments revise the current run only. `learning: true` applies those deliberate learnings and makes no post-run evaluator call. Free-form observation capture requires explicitly writing `capture: { custom: "<guidance>" }` (the built-in evaluator, scoped by your text) or `capture: { agent: ./capture.agentuse }` (an agent file replaces the evaluator; its output still passes the common vet). Setting both is a parse error. `capture: { addons: [tool-errors] }` enables the first typed channel: a failed tool call followed by a corrected call and a confirmed success, verified structurally in code.
- **Learnings carry the contract they were captured against.** Every learning records its capture channel, source session, evidence, and a hash of the effective agent instructions at capture time. When the instructions are rewritten, learnings vetted against the old contract are held out of injection as *stale* rather than injected unexamined, and the next capture or tidy pass re-vets them against the new text: still-consistent rules are re-stamped, conflicting ones are quarantined with the conflict named. Entries from pre-0.19 files carry no hash and are not treated as stale, they stay injectable until the first capture or tidy backfills them (quarantining failures, never deleting; tidy's undo covers the pass). The hash covers your instructions with the machine-managed `<!-- agentuse:learned -->` block excised, so a graduation never invalidates the rest of the store.
...
- **BREAKING: a bare `agentuse serve` no longer adopts the current directory as a project.** Existing deployments that relied on the v0.18 fallback must start with `agentuse serve -C .` (or an explicit path), or persist the directory under `serve.projects`. When a bare launch finds an `.agentuse` file below the current directory but has no configured project, startup now prints this migration command instead of silently serving zero agents and schedules.
- **BREAKING (narrowed, with a safe mapping): `learning: true` and `learning.capture: true` no longer auto-learn from a run or from unchecked reviewer comments.** Free-form auto-capture requires `capture.custom`, `capture.agent`, or a typed add-on. Both forms keep parsing and warn once with the explicit mapping; `agentuse doctor` repeats the notice. `apply`, `max`, and `model` are unchanged; an empty `capture: {}` object equals `capture: true`.
- **Learnings are contextual guidance, not unconditional rules.** Runtime and graduated learning blocks now tell the model to apply a learning only when its situation is relevant, preserve the reviewer's intended scope, and defer to the current task and authored agent instructions. A relevant learning may refine a soft skill default without turning every historical correction into a checklist item.
...
- **Dashboard preferences now state where each choice is saved and no longer compete with native Mac Settings.** Browser-only push and cache controls stay in the browser, Desktop links directly to Mac Settings for app-wide controls, and the duplicate header reload button has been removed.
- **Desktop onboarding and coding-agent handoffs consistently use the CLI bundled inside AgentUse.app.** A different global `agentuse` installation can no longer redirect first-agent setup to the wrong runtime profile, and provider login stays in Terminal rather than passing OAuth URLs, authorization codes, or API keys through coding-agent chat.
- **Desktop-managed servers use the configured `serve.port`, or AgentUse's standard `127.0.0.1:12233` address, when it is available.** The Mac app falls back to an ephemeral loopback port only when another process already occupies the preferred port.
- **Restoring the Desktop dashboard no longer leaves the AgentUse wordmark accidentally focused.** Intentional keyboard focus remains available while hide-and-show cycles return focus to the page content.
- **Opening the Desktop dashboard during startup no longer interrupts its initial page load.** Startup, global-shortcut, Dock, and notification requests are serialized so overlapping requests do not produce an `ERR_ABORTED` launch error.
