...
This release makes the `serve` web UI installable and push-capable: Web Push notifications for approvals and session completions (with Declarative Web Push on iOS and a pending-approvals badge on the app icon), plus Add to Home Screen support. Under the hood, the model registry now carries real context and output limits for every provider (fixing premature compaction and silently truncated responses), `filesystem_read` gains image/PDF input, and a failed compaction no longer silently ends a run. Approval gates, the session/approval web UI, the JSON API, channels/Slack, and the new web push support remain **experimental**: route shapes, UI details, and API response formats may still evolve based on production feedback.
...
- **Web app install support**: a web app manifest, PNG home-screen icons (180/192/512), and install meta tags enable Add to Home Screen on iOS Safari and install on Android. Icons and the manifest are served pre-auth (like the favicon) so install works from token-only session links.
...
- **Every timeout field now accepts suffixed duration strings, and `approval.timeout` bare numbers now mean seconds** (unit unification; one behavior fix). All user-facing timeout fields (`timeout`, `sandbox.timeout`, `mcpServers.*.toolTimeout`, `tools.bash.timeout`, `approval.timeout`) accept duration strings like `"30s"`, `"10m"`, `"24h"`, `"500ms"` via a shared parser; bare numbers keep their historical seconds unit everywhere EXCEPT `tools.bash.timeout`, which now **rejects bare numbers outright** (breaking) with a corrective error naming both readings (`"120s" if 120000 was milliseconds`) - this was the one config field measured in milliseconds while every sibling uses seconds, and neither keeping the ms reading (30 = instant kill) nor flipping it later (120000 = silent 33-hour timeout) is survivable, so the ambiguous form is simply invalid. Migration is one line per agent (`timeout: 120000` → `timeout: "2m"`); rejections emit an anonymous `timeout_unit_error` telemetry event so the real-world migration cost is measurable. Behavior fix: a bare-number `approval.timeout` previously parsed as milliseconds, expiring the gate near-instantly; it now means seconds (no one could have depended on a millisecond approval window on purpose). Invalid durations fail at parse time instead of silently producing a near-zero timeout. The bash tool's model-facing per-call `timeout` parameter also accepts duration strings, and a bare number under 1000 is rejected with a corrective error instead of killing the command at N milliseconds - the seconds-vs-ms guess that used to waste a run now self-heals on the model's next call (a real sub-second timeout is still expressible as `"500ms"`). `mcpServers.*.toolTimeout` and the `MCP_TOOL_TIMEOUT` env fallback are now documented.
