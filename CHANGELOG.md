...
- **Creating and revising an agent now happens in one draft-and-refine workspace.** The authoring session remains open while the operator reviews the source, asks for changes in the same thread, inspects the author's work, and runs the draft through the same test pipeline as `agentuse test` before saving. Suggested ideas and existing-agent revisions open this same workspace rather than separate flows.
