...
<Tip>
  Any model available from Anthropic, OpenAI, OpenRouter, OpenCode Go, or Amazon Bedrock can be used. You can also specify custom API keys: `anthropic:claude-sonnet-4-6:CUSTOM_API_KEY`
</Tip>
...
agentuse run hello.agentuse -m anthropic:claude-sonnet-4-6
```
...
```bash
agentuse run analyzer.agentuse --model anthropic:claude-sonnet-4-6
