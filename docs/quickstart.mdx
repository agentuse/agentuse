...
```bash
agentuse run analyzer.agentuse --model anthropic:claude-sonnet
agentuse run analyzer.agentuse -m openai:gpt-5.4-mini
```

Leaving the version off (`anthropic:claude-sonnet`) tracks the newest model in that line; pin a version when you need reproducibility.
