...
| `CLAUDE_CODE_OAUTH_TOKEN` | Claude Pro/Max | `npx agentuse auth setup-github` | Subscription users; provider plugin required |

<Note>
For Claude Pro/Max users, install the Claude Code subscription provider plugin in the project or runner image, then run `npx agentuse auth setup-github` in your project directory. The command writes the token secret to GitHub; the token alone does not install or activate its transport.
</Note>
...
Before pushing a workflow change, run the agent locally with [`agentuse test`](/guides/testing-agents): it exercises the whole run without posting comments or touching the repository.
