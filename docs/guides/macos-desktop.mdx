...
by the desktop controls. If that external server restarts, Desktop keeps its
port available and reconnects instead of starting a competing server. Use
**Start Server** explicitly if the external server has been retired and you
want Desktop to replace it. Login launches start in the background so the menu-bar
...
AgentUse checks quietly for updates on a recurring schedule while the menu-bar
app is running, without requiring Settings or the dashboard to stay open, and
downloads available updates in the background. When an update is ready, choose
**Restart Now** in the native prompt or **Later** to keep working. You can also
open **Settings → About** to check manually or restart and install a downloaded
update. AgentUse never installs an update without an explicit restart decision.
