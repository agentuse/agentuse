...
<Warning>
  Remote sub-agents are **not currently supported**. Sub-agents can only be loaded from local file paths relative to the parent agent.
</Warning>

All sub-agent paths must be relative to the parent agent's directory:

```yaml
---
model: anthropic:claude-sonnet-4-0
subagents:
  - path: ./translator.agentuse          # Same directory
  - path: ../shared/processor.agentuse   # Parent directory
  - path: ./team/specialist.agentuse     # Subdirectory
---
```
...
model: anthropic:claude-sonnet-4-0
subagents:
  - path: ./team-lead.agentuse  # This has its own sub-agents
---
...
<Note>
  Each level of sub-agents creates its own MCP connections and tool set. Paths are always resolved relative to each agent's directory.
</Note>
