...
| `$AGENTUSE_CONFIG_DIR/skills/` | User | Personal skills across projects; defaults to `~/.agentuse/skills/` |
...
dangling links and resolves directory real paths to prevent circular traversal.
</Note>
...
The stub redirects to `agentuse skills get core`, keeping instructions aligned with the installed CLI version. `core` is the entry point: it points on to `runner` (running, listing, sessions, serve, schedules), `creator` (authoring and reviewing `.agentuse` files), and `tester` (validating agents with mock runs), each fetched the same way.

Dashboard creator sessions can read these version-matched builtins directly,
including `core`, `creator`, and `tester`, without shell access. The creator
guide is already included in the authoring context. Builtin guidance is separate
from the installed skill catalog, and reading it grants no execution tools.
...
| `allowed-tools` | No | Required tools: a space- or comma-separated string, or a YAML list |
| `license` | No | Skill license (e.g., MIT, Apache-2.0) |
| `compatibility` | No | Version requirements or environment notes |
| `metadata` | No | Custom annotations. Values may be strings or nested objects; the runtime never interprets them |

<Note>
`metadata` is free-form on purpose. AgentUse preserves and surfaces these keys
(`agentuse skills installed list --json`) but never reads them, so a skill shared
with another tool can carry that tool's nested config here without AgentUse
rejecting the skill.
</Note>
...
The `allowed-tools` field documents which tools your skill needs, for **documentation purposes** and to drive [skill trust](#trusting-a-skill).

Write it as a space-separated string, a comma-separated string, or a YAML list. All three are accepted:
...
A `Bash(...)` pattern may contain spaces, so multi-word command prefixes work:
`Bash(git commit *)` and `Bash(npm run *)` are split as single patterns, not on
their internal spaces.

The two trailing-wildcard spellings are equivalent, matching Claude Code's
permission syntax: `Bash(ls:*)` and `Bash(ls *)` mean the same thing. A wildcard
anywhere other than the end (`Bash(git * main)`) is kept as documentation but
grants nothing, because narrowing it to a prefix would grant more than the
pattern describes.
...
| `gated` | `string[]` | Command patterns that require an approved per-action lease |
| `allowedPaths` | `string[]` | Additional directories accessible beyond project root. Supports `~` for home directory. |
| `timeout` | `string` | Command timeout as a suffixed duration such as `"30s"`, `"2m"`, or `"500ms"`; bare numbers are rejected |
...
```yaml
skills:
  linkedin:         # bare = include, no special grant
  auto: false       # ...and hide every other discovered skill
```

Because hiding skills is the rarer, higher-consequence choice, it is loud (`auto: false`) rather than a silent side effect of listing.
...
| Config | Discovery | Preloaded | Trusted |
|--------|-----------|-----------|---------|
| omitted / `skills: auto` | all | none | none |
| `skills: [linkedin]` | all | linkedin | none |
| `skills: { linkedin: trusted }` | all | linkedin | linkedin |
| `skills: { linkedin:, auto: false }` | **only linkedin** | linkedin | none |
| `skills: trusted` | all | none | all |
