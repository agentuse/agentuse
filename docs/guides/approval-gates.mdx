...
Slack is configured as a notification channel, not inside `approval`.
...
Then the agent can omit `channel_id`:
...
Slack approval messages are compact. They show the prompt, session/project metadata, status, and a `Review approval` button. The full review context stays on the AgentUse approval page.

For completion and failure notifications, or more route examples, see [Notifications](/guides/notifications).
