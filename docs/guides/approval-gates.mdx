...
<Note>
A leaf agent's approval gate works the **same whether the agent is run directly or delegated by a `type: manager`** agent. See [Approval in delegated sub-agents](#approval-in-delegated-sub-agents) below.
</Note>
...
tools:
  bash:
    commands:        # auto-run, no approval
      - "birdc read *"
      - "birdc search *"
    gated:           # run only after you approve the exact action
      - "birdc reply *"
```
...
Declaring `gated` enables the approval machinery automatically, you do not also need `approval: true`. For a plain yes/no decision, the agent emits `await_human` and the gated command together. AgentUse attaches the exact command to the request, blocks it from executing, and lets the agent re-issue it only after you approve.
...
3. Each approved entry is one-shot. It is consumed before dispatch, so an ambiguous failed external action is never retried silently. Listing the same exact command twice authorizes two executions.
...
- **Plain gates auto-attach the command.** Emit `await_human` and the exact gated bash command in the same model step. The command is shown to the reviewer but remains blocked until approval; after approval the agent re-issues it once. Non-gated tools still cannot ride beside a gate.
- **Pick gates scope actions explicitly.** Give each `changes[]` action an `optionId` referencing the corresponding `options[].id`. When `content` is an executable command, also set `displayContent` to the exact human-facing post, reply, message, or body. The reviewer sees the business content first with the command de-emphasized inside the same option. Approval grants only the selected option plus any action with no `optionId`. This is the safe one-approval pattern for choosing among several irreversible commands.
- **Show the complete command.** Whether auto-attached or option-scoped, the lease covers the exact executable command (`birdc reply "text"`), not payload text alone. For browser automation, gate and show the step that carries the words (`agent-browser type "..."`), not a contentless click.
...
- `reference`: the original item the action responds to (`{ label, author, title, url, excerpt }`), such as the post being commented on or the message being replied to. On a wide screen it renders in a column beside the changes, so the reviewer reads original and response together; on a narrow one it stacks above them
- `options`: two or more choices for the reviewer, each with an `id`, `label`,
  and optional `description` / `recommended`. The session page renders a
  single-choice picker before the decision controls. On approval, the selected
  id reaches the agent as `toolResult.choice`
...
### The original is mandatory on a response gate

When the gate approves a reply, comment, answer, or any other response to
something someone else wrote, `reference` is required and its `excerpt` must
carry the **complete verbatim original**, not a summary and not the first few
lines. The schema rejects three shapes that force the reviewer to leave the
card and open the source:

- a response gate with no `reference` at all (putting the original in `context`
  prose does not count)
- a `reference` that carries an author, title, or `url` but no `excerpt`
- an `excerpt` with an explicit elision marker such as `[truncated]` or
  `(full text at the link)`

A gate for a *fresh* post, email, or document is unaffected: it answers nothing,
so it has no original to quote.
...
`Comment` is the revise-and-re-gate path, not a failed selection. On a choice
gate, an actionable edit or direct replacement in the comment takes precedence
over the absence of a selected option. For example, `reply "very nice!"` means
to replace the proposed reply with `very nice!` and request approval again; it
is not ambiguous merely because it names no option. Only an explicit request to
cancel, abandon, or stop makes a comment terminal.

For a choice gate, AgentUse preselects the recommended option, or the first
option when none is recommended. The reviewer can change that selection before
approving. The result retains the normal approval status and adds the selected
option id:
...
Use stable machine-readable ids. Labels and descriptions may change without
breaking the agent logic that consumes `toolResult.choice`.
...
```md title="reply-to-post.agentuse — the leaf, reused in both modes"
---
model: anthropic:claude-sonnet-5
approval: true
---

Draft a reply to the given post in the brand voice, then publish it.
```
...
```md title="manager.agentuse — delegates to the same leaf, unchanged"
---
type: manager
schedule: "0 9 * * *"
subagents:
  - path: ./reply-to-post.agentuse
---

For each post worth replying to, delegate to reply-to-post.
```
...
- `tools.bash.gated`: bash command patterns that run only after human approval, enforced by a per-action lease. Declaring it implies the approval gate. See [Gated commands](#gated-commands-mechanical-enforcement).
...
```bash
curl -X POST "http://127.0.0.1:12233/sessions/session_abc123/reopen?token=$sessionToken" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
### Testing gate flows unattended

Resuming by hand is fine for one gate. To check that each branch behaves, let `agentuse test` play the reviewer deterministically:

```bash
agentuse test my-agent.agentuse --approval approve
agentuse test my-agent.agentuse --approval reject
agentuse test my-agent.agentuse --approval comment:"tighten the subject line"
```

The run finishes with no human involved, and an approval grants the gated-command lease exactly like a real reviewer, so `tools.bash.gated` flows complete end to end without executing anything irreversible. See [Testing Agents](/guides/testing-agents).
