...
The Slack channel message stays concise; summary, draft, artifact, context, risk, and approval actions are posted in the message thread when available.
...
When Slack Socket Mode is configured, a normal reply in the approval notification thread also resumes the agent as a `comment` decision.
