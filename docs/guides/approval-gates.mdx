...
2. A gated command never runs anything other than what you approved. Approval binds to the exact content the reviewer saw (the tweet text, the command); if the agent revises it, the old approval no longer covers it and it must re-gate.
...
- **Gate the content-bearing step.** The verbatim-content guarantee works when the approved content is in the command itself (`birdc reply "text"`). For browser automation, gate the step that carries the words (`agent-browser type "..."`), not a contentless click.
...
- `artifact_path` / `artifact_paths`: project-relative local files the session page can open in the artifact viewer. Images, HTML, and PDF artifacts also render as inline previews in the approval card
