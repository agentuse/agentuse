...
<Warning>
  Channel event names and the `channels` configuration contract are
  experimental and may change without migration as AgentUse adds destinations.
</Warning>
...
Approval channel threads can also accept decisions. For Approve/Reject buttons and comment-by-thread-reply, configure Socket Mode:
