...
- `$AGENTUSE_CONFIG_DIR/plugins/*.{ts,js}` - User-global plugins (loaded second; defaults to `~/.agentuse/plugins/*.{ts,js}`)
...
```typescript
{
  agent: {
    ...
    model: string;        // Model used (e.g., "anthropic:claude-sonnet-5")
...
```
📊 Performance Traces:
  🧠 anthropic:claude-sonnet-5: 1234ms
     └─ Tokens: 450
  🔧 bash: 567ms
  🤖 code-reviewer: 8901ms
     └─ Tokens: 1200
  🧠 anthropic:claude-sonnet-5: 2345ms
     └─ Tokens: 320
