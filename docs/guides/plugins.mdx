...
Browse the current choices in **Settings > Providers**. Each provider card
shows its publisher, reviewed version, installation source, and authentication
requirements. Community providers maintain their own setup instructions;
follow the linked repository for provider-specific credentials and limitations.
Some providers use credentials managed by an external application rather than
an AgentUse login.
