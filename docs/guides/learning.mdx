...
1. **Supersede.** The evaluator sees every rule the agent already carries, each
   with its id, and is asked to reconcile rather than merely avoid duplicating.
   If its new learning restates, sharpens, or *contradicts* an existing rule, it
   returns one rule that resolves the collision, naming the id it replaces. The
   replaced rule is retired, not deleted.
2. **Trade.** Otherwise the weakest auto-extracted rule is retired to make room -
   but only if the new rule is worth more than the one it displaces, so the set
   keeps the best rules rather than the last ones.
3. **Ask you.** If nothing may be dropped, because every rule in the set is a
   human correction, an auto-extracted learning is refused and *your* correction
   is kept anyway, over the cap. Your corrections are never discarded to satisfy
   the system's own bookkeeping.

Two things are never evicted: a rule you wrote or approved, and a rule you have
repeated. A repeat means the wording is not landing, which is a reason to rewrite
it, not to drop it.
...
Case 3 is the one worth interrupting for, and the run says so:

```
learned guidelines: 15 of 19 applied, ~1.8k tokens/model request
This agent holds 4 correction(s) beyond its 15-rule limit, and all of them are
human corrections - nothing can be dropped automatically.
Fold them into the agent's instructions: agentuse learnings tidy ./blog-writer.agentuse
```

That is not a storage problem. It means your agent file does not describe the
job, and the corrections are carrying what the instructions should have said.
Only you can move them there.
