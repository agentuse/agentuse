...
A pass reads every stored correction, so it takes a few minutes. In the web UI it
runs on its own page, reporting which batch it is on; leaving that page does not
stop it, and the result waits there with its Undo button. The agent's Learnings
