...
When [Approval Gates](/guides/approval-gates) are enabled and `capture` is on, approving a gate **with a comment** can promote that comment into a durable learning. This is the highest-signal feedback an agent receives, so it is captured separately from self-evaluation.
...
When you approve and leave a note like "Going forward, always cite a source before publishing", a generalizability filter decides whether the comment is a **durable agent-wide rule** or a **one-off edit** to this run. Only durable rules are captured (as `src:approval`, confidence `0.95`); one-off edits are ignored. Approval-sourced learnings rank ahead of auto-extracted ones when injected, so your corrections survive the per-run cap.
