...
- `--no-mock` - Hide mock/test runs. The CLI lists them by default, marked `· mock`
...
- `/sessions` lists every run, newest first. Filter the list with `?agent=<id>`,
  `?status=running|suspended|completed|error|incomplete`,
  `?trigger=scheduled|manual|slack|api`, `?days=<n|all>`, and
  `?mock=include|only`. Mock/test runs are hidden by default; `include` adds
  them to the list and `only` shows nothing else. A mock session's detail page
  is always reachable by id, filter or not.
...
### Internal agent-revision sessions

A completed, failed, or approval-paused run can start a reviewable agent
revision. The original session remains the launch point and durable history,
with a link to each revision. An agent that has never run can instead start from
its detail page and use the current agent file as evidence. The revision session
links back to its origin when one exists and holds the diagnosis, source
proposal or no-change result, reviewer feedback, apply decision, and restore
action.

The Sessions list and feed label these AgentUse-owned jobs as `internal
revision`, group them under **Internal revisions**, and name them **Revising
_agent_**. They are distinct from delegated subagents and automated Judge
sessions. See [Agent Revisions](/guides/agent-revisions) for the full review and
write-safety workflow.

For delegated runs, the parent timeline also surfaces important work from nested
descendants. Reviewer comments appear as timestamped feedback; a child working
on that feedback is labeled `revising`, and its next gate is labeled
`awaiting-approval`. Judge sessions are identified as automated pre-review
attempts so they are not mistaken for human decisions.

The `/agents` page opens each agent's detail hub, where you can inspect source,
recent runs and revisions, and start a run. Schedule rows also link to the agent
detail hub; last-run badges still link to the matching session page.
...
If a session is stuck at `running` long after its process is gone, usually after a crashed daemon or a killed container, settle it with [`agentuse sessions reconcile`](/reference/cli-commands#reconcile-stuck-sessions). Runs that are still executing are never touched.
...
- [Environment Variables](/reference/environment-variables#storage) - Configure storage location with `AGENTUSE_DATA_DIR`
