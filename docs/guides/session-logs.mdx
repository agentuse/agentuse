...
with a link to each revision. The revision session links back to that origin and
holds the diagnosis, source proposal or no-change result, reviewer feedback,
apply decision, and restore action.
