...
One `agentuse serve` process can host agents from several project directories. Repeat the `-C` flag:

```bash
agentuse serve -C ./projA -C ./projB
```

Project ids default to the directory basename (e.g. `projA`, `projB`). Two projects with the same basename are a hard startup error; rename one or serve them separately.

Send requests with a `project` field in the body:
...
Omit `project` and the server returns `400 PROJECT_REQUIRED` with the list of available ids, unless you set a default at startup:

```bash
agentuse serve -C ./projA -C ./projB --default projA
```

With `--default`, requests without a `project` field route to the default project. Explicit `project` fields still win.

### Environment isolation

Each project has its own `.env` / `.env.local`; they never mix. One subprocess worker per project owns that project's environment. No project-level env is loaded into the parent `serve` process.

## API Reference

### GET /

Server info for discovery. Returns version, default project (if any), and the list of served projects.
