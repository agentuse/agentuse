...
<Warning>
  The `sandbox` configuration and container execution contract are experimental
  and may change without migration. This label is not a security guarantee; use
  the isolation boundaries documented below when evaluating untrusted work.
</Warning>
...
sandbox:                       # full config
  ...
  timeout: 10m               # default: 300 seconds
...
| `timeout` | `number \| string` | `300` | Max container lifetime. Bare numbers mean seconds; duration strings such as `"10m"` are accepted |
