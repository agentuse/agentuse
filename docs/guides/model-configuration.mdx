...
## Naming a Model Without a Version

Pinning an exact model id means editing every agent file when a new release
ships. Three ways to avoid that, in increasing order of scope:

**1. Drop the version.** A model id with its version left off means "the newest
in this line":
...
The lineup behind these aliases is refreshed each AgentUse release, so an alias
always points at a model the release shipped with. A real model id always wins
over an alias, so anything you pin keeps its exact meaning. See
[Version Aliases](/reference/models#version-aliases) for the current table.

**2. Name your own.** A `models.aliases` block in `~/.agentuse/config.json` lets
one edit repoint every agent that uses it:

```json
{
  "models": {
    "aliases": { "fast": "anthropic:claude-haiku", "smart": "anthropic:claude-opus" }
  }
}
```
...
An alias can also carry an ordered fallback list:
...
AgentUse falls back when an execution segment cannot get started on a candidate:
a transient first-request failure (rate limit, timeout, provider 5xx, transport
error), or a model it cannot authenticate to at all (no login, an OAuth token
that will not refresh, a 401/403). This includes a segment resumed after an
approval gate. Either way it only happens before the model emits new output or
invokes a tool in that segment. The selected model and remaining ordered
fallback policy are persisted for later resumes. `cooldown` keeps new runs in
the same process
from repeatedly waiting on a model that just failed; it resets when the process
restarts.

**3. Set a default and stop naming models.** With `models.default` (or
`AGENTUSE_MODEL`) configured, `model:` becomes optional:

```json
{ "models": { "default": "anthropic:claude-sonnet" } }
```

Aliases resolve to concrete models before execution, so session logs and cost
accounting record the model that actually ran. A suspended run first resumes on
its last selected model, even if the alias has moved since; a fallback-eligible
failure before new output may then advance through its persisted policy.

To migrate agent files you already have, `agentuse models unpin` swaps pinned
versions for aliases (`--dry-run` to preview), and `agentuse models bump` moves
stale pins forward for files you want to keep pinned.
...
Anthropic API keys remain built in. Claude Pro/Max OAuth is supplied by the
shortlisted community plugin and can be installed from provider setup or from
the CLI:

```bash
agentuse plugins install cb7337/agentuse-claude-code-provider@v0.1.0
agentuse provider login anthropic
```

Users upgrading with an Anthropic OAuth credential created by the former core
flow do not need to run these commands. AgentUse installs the pinned
compatibility plugin and adopts that credential automatically. The explicit
community-plugin installation flow is shown only for new configuration.

Community providers that use an external CLI can also appear during onboarding
when their required executable is on the AgentUse server's PATH. Installing one
uses that CLI's existing credentials and runs the provider's readiness check
before continuing. If the check fails, setup shows the provider's error and
setup instructions. Installed CLI providers remain available for retrying setup.
...
agentuse run agent.md --model openai:gpt-5.6:OPENAI_API_KEY_PERSONAL
...
Connect hosted or local endpoints from **Dashboard Settings → Providers**. A
custom provider can use OpenAI Chat Completions, OpenAI Responses, or Anthropic
Messages. Dashboard setup discovers available models, lets you choose which to
save, and verifies the selected protocol with a minimal completion before the
provider is ready. Saved custom providers remain editable in Settings.

The CLI supports adding OpenAI-compatible endpoints such as Ollama, LM Studio,
vLLM, and llama.cpp.
...
Stored credentials:
  🔑 anthropic (Claude subscription OAuth) → Use as: anthropic:claude-sonnet-5
  🎫 openai (api) → Use as: openai:gpt-5.6

Environment variables:
  🌍 openrouter (OPENROUTER_API_KEY) → Use as: openrouter:z-ai/glm-5.3
...
agentuse run agent.agentuse -m opencode-go:kimi-k2.7-code
agentuse run agent.agentuse -m anthropic:claude-sonnet   # version alias
agentuse run agent.agentuse -m @fast                     # configured alias
```

Without `--model`, a parent and each delegated sub-agent use their own declared
models. An explicit runtime override is run-wide: it applies at every sub-agent
depth, including the full candidate order of a fallback alias, and is preserved
...
---
name: powerful-agent
model: openai:gpt-5.6  # Uses OpenAI GPT-5
openai:
...
model: openrouter:z-ai/glm-5.3  # Uses OpenRouter
---
```
...
### Reasoning Effort (recommended, provider-agnostic)

The simplest way to control how hard a model thinks is the top-level
`reasoning`. AgentUse normalizes the level to the selected model's supported
vocabulary, then applies the corresponding provider control. For example,
GPT-5.6 maps `minimal` to `low`, while modern Claude uses adaptive thinking, so
one setting works whether the agent runs on Claude or GPT-5:
...
and `anthropic.thinking.budgetTokens` below are escape hatches for exact wire-level control,
...
openai:
  reasoningEffort: high      # Options: 'none', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max'
