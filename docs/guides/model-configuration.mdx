...
Aliases resolve once, when the agent file is parsed, so session logs and cost
accounting record the concrete model that ran. A suspended run resumes on the
