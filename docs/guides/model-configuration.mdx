...
- name: Run agent (Development)
  run: agentuse run agent.agentuse --model anthropic:claude-haiku-4-5
  if: github.ref != 'refs/heads/main'

- name: Run agent (Production)
  run: agentuse run agent.agentuse --model anthropic:claude-sonnet-4-5
  if: github.ref == 'refs/heads/main'
```

```bash
# Shell script for model comparison
for model in "anthropic:claude-haiku-4-5" "openai:gpt-5.2" "openai:gpt-5-mini"; do
