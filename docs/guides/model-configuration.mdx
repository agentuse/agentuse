...
AgentUse falls back only when a fresh session's first model request fails for a
transient reason before emitting output or invoking a tool. The selected model
is then fixed for that session. `cooldown` keeps new runs in the same process
