...
AgentUse falls back when a fresh session cannot get started on a candidate:
...
that will not refresh, a 401/403). Either way it only happens before the model
emits output or invokes a tool, and the selected model is then fixed for that
session. `cooldown` keeps new runs in the same process
...
accounting record the model that actually ran. A suspended run resumes on the
model it started with, even if the alias has moved since.
