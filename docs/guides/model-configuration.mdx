...
`reasoningEffort`. It works across providers, AgentUse maps one level to each
...
Reach for `reasoningEffort` first. The provider-specific `openai.reasoningEffort`
and `anthropic.thinking.budgetTokens` below are escape hatches for exact control,
honored only when the top-level `reasoningEffort` is unset. Being top-level,
`reasoningEffort` also avoids a subtle trap: a provider-specific key placed at
