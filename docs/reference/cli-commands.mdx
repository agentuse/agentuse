...
agentuse run agent.agentuse --model anthropic:claude-sonnet-4-6:ANTHROPIC_API_KEY_PERSONAL
```
...
-C, --directory <path>  Project directory (repeat for multi-project)
--default <id>          In multi-project mode, project id to route POST /run when no `project` is supplied
...
One server process can host agents from several project directories. Repeat `-C`:

```bash
agentuse serve -C ./projA -C ./projB
```

Project ids are derived from the directory basename (`projA`, `projB`). Two projects with the same basename are a hard startup error; rename one or serve them separately. Each project keeps its own `.env` / `.env.local`; envs never mix across projects.

Requests include a `project` field in the body to pick which project runs the agent. In single-project mode, the field is ignored, so existing integrations work untouched. In multi-project mode, omitting `project` returns `400 PROJECT_REQUIRED` unless `--default <id>` is set at startup, in which case requests without `project` route to the default.
...
`default` is `null` when no `--default` is set (or in single-project mode, the sole project id).
...
| `project` | string | Conditional | Project id. Optional in single-project mode. Required in multi-project mode unless `--default` is set |
...
|------------|-------------|-------------|
| `NOT_FOUND` | 404 | Endpoint not found (use `POST /run` or `GET /`) |
| `AGENT_NOT_FOUND` | 404 | Agent file doesn't exist |
| `PROJECT_NOT_FOUND` | 404 | Unknown project id in multi-project mode |
| `PROJECT_REQUIRED` | 400 | Multi-project mode with no `project` field and no `--default` |
...
# Same, with POST /run defaulting to projA when `project` is not supplied
...
Shows PID, port, project summary, total agent/schedule counts, and uptime for each server. Multi-project servers show the first project id followed by `+N`, where N is the count of additional projects.

```
PID      PORT     PROJECTS                       AGENTS   SCHEDULES   UPTIME
────────────────────────────────────────────────────────────────────────────
63398    12233    ~/workspace/agentuse           9        1           2h 15m
71245    12234    projA +2                       12       3           45m
```
...
Then use: `agentuse run agent.agentuse -m ollama:glm-4.7-flash:q4_K_M`
