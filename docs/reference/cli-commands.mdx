...
- **Stores are isolated under mock**: reads are seeded by copying the real store, writes land in `<projectRoot>/.agentuse/store-mock/<run-id>/` (kept for inspection), and the real store, including the reserved `metrics` store behind dashboards, is never touched.
...
Inspect and tidy an agent's stored corrections. Only the top few apply per run (10 by default); anything past that is stored but never reaches the model. See the [Learning guide](/guides/learning).
...
| Command | Description |
|---------|-------------|
| *(none)* | List every correction by status: applied, never reaches the agent, in the agent file, retired. |
| `tidy` | Merge near-duplicates, sharpen corrections you have repeated, retire what is superseded, and make proven corrections permanent in the agent file. Applies immediately, then shows both diffs. Keeps making passes until another one would not help, up to five, and writes both files once at the end, so one run is one undo. Says what it is doing as it goes: a long list takes a few minutes. If it ends still over the cap, it accounts for every correction it left in force. |
| `undo` | Restore the corrections file **and** the agent file to their state before the last tidy-up. |
| `migrate` | Move corrections written before v0.17.0 from beside the agent file into the AgentUse state directory. Moves by default; refuses a destination that already holds corrections rather than merging; also lists any `*.learnings.md` with no matching agent file as orphaned. |

### Options

| Option | Description |
|--------|-------------|
| `-j, --json` | Machine-readable listing, including each rule's state and evidence counters. The `file` field is the resolved corrections path. |
| `--path` | Print the path of the corrections file and exit. |
| `--edit` | Open the corrections file in `$EDITOR` (or `$VISUAL`), creating its directory if needed. |
...
| `--keep-source` | (`migrate`) Copy instead of moving, leaving the old file in place. |
...
# Where does this agent's corrections file live?
...
<Note>
  A tidy-up writes proven corrections into a marked block in your `.agentuse` file. Only the bytes inside the markers change, frontmatter, comments and formatting are left exactly as they were, and `undo` restores both files byte for byte.
</Note>

<Warning>
  Corrections no longer live beside your agent file. As of v0.17.0 they live under the AgentUse state directory, and the old location is never read. Run `agentuse learnings migrate --all` once per repo to move them, then commit the deletions. `migrate` exits non-zero if it refused any file.
</Warning>
...
agentuse learnings <file> [--json]       # List an agent's stored corrections by status
agentuse learnings <file> --path         # Print where the corrections file lives
...
agentuse learnings migrate --all         # Move pre-0.17.0 corrections into the state directory
