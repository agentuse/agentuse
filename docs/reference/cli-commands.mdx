...
`--remember` saves a separate future learning rule and requires `--comment` (a non-empty one). Passing `--remember` is itself the opt-in, so saving needs no `learning` config on the agent. The comment is still the feedback for this revision; the remembered rule is stored as `src:manual` and is injected into future runs once the agent has `learning.apply` enabled.
