...
Agents with a `schedule` config in their frontmatter are automatically loaded and executed on schedule:

```yaml
---
model: anthropic:claude-sonnet-4-5
schedule:
  every: "hourly"          # Natural language aliases
  # cron: "0 * * * *"      # Or cron expressions
  # interval: "30m"        # Or intervals (5s, 10m, 2h, 1d)
  timezone: "UTC"          # Optional timezone
---
```

On startup, the server scans for `.agentuse` files with schedules and displays them:

```
Scheduled (2)
  Agent                  Schedule        Next Run
  reports/daily.agentuse 0 0 * * *      2024-01-02 00:00:00
  monitors/check.agentuse */15 * * * *  2024-01-01 12:15:00
```

<Info>
  The server enables CORS by default, allowing requests from any origin.
  This is suitable for development and internal tools.
</Info>

<Warning>
  For production use, consider running behind a reverse proxy (nginx, Caddy)
  with proper authentication and rate limiting. The default host `127.0.0.1`
...
ANTHROPIC_API_KEY_DEV=sk-ant-...  # For model: anthropic:claude-opus-4-5:dev
OPENAI_API_KEY_PROD=sk-...        # For model: openai:gpt-5.2:prod
