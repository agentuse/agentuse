...
stable AgentUse version. The dashboard uses it for its dismissible update banner.
...
Inspect and tidy an agent's stored learnings. Only the top few apply per run (15 by default, `learning.max`); anything past that is stored but never reaches the model. See the [Learning guide](/guides/learning).
...
| Command | Description |
|---------|-------------|
| *(none)* | List every learning by status: applied, never reaches the agent, in the agent file, retired. |
| `tidy` | Merge near-duplicates, sharpen learnings you have repeated, retire what is superseded, and make proven learnings permanent in the agent file. Applies immediately, then shows both diffs. Keeps making passes until another one would not help, up to five, and writes both files once at the end, so one run is one undo. Says what it is doing as it goes: a long list takes a few minutes. If it ends still over the cap, it accounts for every learning it left in force. |
| `undo` | Restore the learnings file **and** the agent file to their state before the last tidy-up. |
...
| Option | Description |
|--------|-------------|
| `-j, --json` | Machine-readable listing, including each rule's state and evidence counters. The `file` field is the resolved learnings path. |
