...
AgentUse automatically detects your project root by searching upward for project markers. This ensures that relative paths in your agent files work correctly regardless of where you run the command.

#### Project Root Detection

AgentUse automatically detects project root by searching upward for `.git/`, `.agentuse/`, or `package.json`. This directory is used for loading `.env` files and plugins.

#### Directory Override (-C, --directory)

The `-C` or `--directory` option lets you explicitly set the project root:

```bash
# Run from anywhere, use specific project root
...
# Run agent from subdirectory (finds project root automatically)
cd agents/subfolder
agentuse run my-agent.agentuse  # Still finds .env and plugins at project root

# Override project directory
agentuse run agent.agentuse -C /my/project
