...
Inspect and tidy an agent's stored learnings. Only the top few apply per run (10 by default); anything past that is stored but never reaches the model. See the [Learning guide](/guides/learning).
...
| `migrate` | Move learnings written before v0.17.0 from beside the agent file into the AgentUse state directory. Moves by default; refuses a destination that already holds learnings rather than merging; also lists any `*.learnings.md` with no matching agent file as orphaned. |
...
<Warning>
  Learnings no longer live beside your agent file. As of v0.17.0 they live under the AgentUse state directory, and the old location is never read. Run `agentuse learnings migrate --all` once per repo to move them, then commit the deletions. `migrate` exits non-zero if it refused any file.
</Warning>
