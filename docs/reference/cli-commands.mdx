...
`--remember` saves a separate future learning rule and requires `--comment` plus an agent with `learning.apply` enabled. The comment is still the feedback for this revision; the remembered rule is what gets injected into future runs.
