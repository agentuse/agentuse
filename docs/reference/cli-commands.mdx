...
--mock                  Mock all tool outputs with the LLM instead of executing them (no real side effects)
--mock-model <model>    Model used to generate mock tool outputs (default: the agent model)
...
```bash
agentuse run agent.agentuse --mock                          # mock every tool output
agentuse run agent.agentuse --mock --mock-model demo:default # zero-cost mock model
```

- **Mock model** defaults to the agent's model; override with `--mock-model` (or `AGENTUSE_MOCK_MODEL`). The mock model is still a real LLM call, so credentials are required — use `demo:default` for zero cost.
