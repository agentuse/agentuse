...
```bash
agentuse learnings <agent-file> [--json]
agentuse learnings tidy <agent-file> [--dry-run]
...
| Option | Description |
|--------|-------------|
| `-j, --json` | Machine-readable listing, including each rule's state and evidence counters. |
| `--dry-run` | (`tidy`) Show the plan and both diffs without writing anything. |
