...
- **`--mock-model` is required.** Mock fires an LLM call for *every* tool result, so it must run on a model you can reach. It is deliberately **not** defaulted to the agent's own model: that ran mock onto the agent's premium, rate-limited token and produced opaque `429`s. Set it per run, or once via `AGENTUSE_MOCK_MODEL`.
...
daily.agentuse  Daily Digest  anthropic:claude-sonnet-4-6  0 9 * * *
```
