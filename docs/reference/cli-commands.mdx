...
Add `-l` to any subcommand to use project-local package settings. `install` accepts `owner/repo`, `github:owner/repo`, `git:github.com/owner/repo@ref`, HTTPS or SSH GitHub URLs, and local Git checkout paths. `update` without a name updates every plugin in the selected scope. The shorter `agentuse install|update|list|remove` commands remain as compatibility aliases.
