...
Repeat `-C` to serve multiple project roots:

```bash
agentuse serve -C ./projA -C ./projB
```

Project ids default to directory basenames. Duplicate ids fail startup. Each project loads its own `.env` / `.env.local` in its worker process.
