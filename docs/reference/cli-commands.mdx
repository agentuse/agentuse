...
```bash
-s, --subagents          Include subagent sessions when listing
...
--default <id>          Default project for POST /run when `project` is omitted
...
Repeat `-C` to serve multiple scopes/projects:
...
Returns version, default project, and served projects.
...
# Provide Slack credentials for agents that enable Slack notifications
...
# Default POST /run to projA when `project` is omitted
...
Shows PID, port, project summary, total agent/schedule counts, and uptime. Multi-project servers show the first project id plus `+N` additional projects.
