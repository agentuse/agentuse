...
<ParamField path="approval" type="boolean | object" required={false}>
  ...
  **Fields:**
  ...
  - `url`: webhook URL for the approval notification
  - `channel_id`: Slack channel id for approval messages
  - `timeout`: suspension timeout such as `24h` or `7d`
  - `actions`: reviewer actions; defaults to `approve`, `reject`, and `comment`
  - `include`: what the agent should include in the approval request, such as `draft` and `summary`
  ...
  See [Approval Gates](/guides/approval-gates) for the full setup guide, webhook payload, and resume examples.
</ParamField>
