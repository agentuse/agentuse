...
<ParamField path="reasoning" type='"none" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max"' required={false}>
  ...
  works across Claude and OpenAI without forwarding incompatible values.

  ```yaml
  model: anthropic:claude-opus-5
  reasoning: high   # none | minimal | low | medium | high | xhigh | max
  ```
...
<ParamField path="openai" type="object" required={false}>
  ...
  **Supported Options:**
  - `reasoningEffort`: Controls thinking effort for reasoning models (`'none'`, `'minimal'`, `'low'`, `'medium'`, `'high'`, `'xhigh'`, `'max'`)
...
8. **OpenAI Options**:
   ...
   - `reasoningEffort` must be one of: `'none'`, `'minimal'`, `'low'`, `'medium'`, `'high'`, `'xhigh'`, `'max'`
