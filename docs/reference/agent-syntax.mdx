...
<ParamField path="notifications" type="object" required={false}>
  ...
  **Fields:**
  - `notify_on`: events that should send notifications. `approval` is supported now; `completion` is reserved for broader notification workflows.
  - `channels.slack.enabled`: enable Slack notifications for selected events
