...
<ParamField path="model" type="string" required>
  ...
  **Version aliases.** Leave the version off and the agent tracks the newest
  model in that line, so it needs no edit when a new release ships:

  ```yaml
  model: anthropic:claude-sonnet   # newest Sonnet
  model: openai:gpt-mini           # newest small GPT
  model: openrouter:x-ai/grok      # newest Grok
  ```

  A real model id always wins over an alias, so anything you pin keeps its exact
  meaning. See [Version Aliases](/reference/models#version-aliases) for the full
  table, and run `agentuse models` to see what each alias resolves to today.

  **Named aliases.** `@name` resolves through the `models.aliases` block of your
  AgentUse config, which lets many agent files be repointed at once:

  ```yaml
  model: "@fast"
  ```

  A named alias may be configured with ordered fallback candidates. Fallback is
  ...
  **Omitting `model`.** With `models.default` (or `AGENTUSE_MODEL`) configured,
  the field is optional and the agent uses that default. Without a default, a
  file that omits `model` fails to parse. See
  [Configuration Files](/reference/configuration-files#model-defaults-and-aliases).

  You can also specify a custom environment variable suffix, which composes with
  aliases:
  ```yaml
  model: anthropic:claude-sonnet-5:dev  # Uses ANTHROPIC_API_KEY_DEV
  model: anthropic:claude-sonnet:dev    # Newest Sonnet, ANTHROPIC_API_KEY_DEV
...
<ParamField path="timeout" type="number | string" required={false}>
  Maximum execution time before the agent is terminated. A bare number means
  seconds; a suffixed duration string (`"90s"`, `"10m"`, `"1h"`) also works.
  ...
  **Precedence:** CLI `--timeout` flag overrides this value.
  ...
  ```yaml
  timeout: 600   # 10 minutes (bare number = seconds)
  timeout: 30m   # duration string
  ```
...
<ParamField path="maxOutputTokens" type="number" required={false}>
  Maximum tokens the model may generate in a **single response** (the provider's
  `max_tokens`). This is a per-step ceiling, not a run-wide total.

  Leave it unset in almost all cases. When unset, first-class Anthropic models
  ...
  to the model's real output limit when that limit is known.

  ```yaml
  maxOutputTokens: 64000   # allow a bigger single response / file write
  ```

  <Note>
    This exists because a model id newer than the underlying SDK's model table
    (e.g. `claude-sonnet-5`) would otherwise be capped at a tiny 4096 `max_tokens`,
...
<ParamField path="intent" type="boolean" required={false} default="true">
  Tool-call intent phrases. When enabled (the default), every tool schema gains
  an optional `intent` parameter: one short phrase from the model stating what
  ...
  tool never sees the parameter (it is stripped before dispatch).

  Set `intent: false` to keep tool schemas pristine, for example when a provider
  or a third-party MCP server is sensitive to extra parameters, or to save the
  few output tokens the phrase costs on every call.

  ```yaml
  intent: false   # disable intent phrases for this agent
  ```
</ParamField>
...
<ParamField path="metadata" type="object" required={false}>
  ...
  Metadata is shown by `agentuse agents`: top-level keys render as chips after
  ...
  `agentuse agents --json` under `.metadata` for filtering.

  ```bash
  # List only draft agents
  agentuse agents --json | jq '.agents[] | select(.metadata.draft)'
  ```

  <Note>
    Metadata is an annotation, not runtime input: it is not injected into the
    agent's prompt.
  </Note>
</ParamField>
...
<ParamField path="dependsOn" type="string | string[]" required={false}>
  ...
  ```yaml
  dependsOn:
    - ./collect-sources.agentuse
    - ./normalize-records.agentuse
  ```

  `dependsOn` is advisory metadata. AgentUse exposes the relationship through
  ...
  A single dependency can use the string shorthand:

  ```yaml
  dependsOn: ./collect-sources.agentuse
  ```
</ParamField>

<ParamField path="learning" type="boolean | object" required={false}>
  Let the agent keep what reviewers tell it and apply the best of it to later
  runs. See the [Learning guide](/guides/learning).

  `learning: true` is shorthand for `{ capture: true, apply: true }`, which
  captures **human corrections only**. Free-form observation capture is opt-in
  through `capture.custom` or `capture.agent`.

  ```yaml
  learning:
    capture:
      addons: [tool-errors]     # typed, structurally-verified channels
      custom: |                 # or: agent: ./capture.agentuse
        Only keep instructions about tone and sourcing.
    apply: true
    max: 20
  ...
  |-----|------|-------------|
  | `capture` | boolean \| object | `false` disables capture. `true` (or `{}`) captures human corrections only: approval comments, `--remember`, manual add. The object form adds channels. Default `true` |
  | `capture.addons` | string[] | Channels verified structurally in code rather than by prompt. `tool-errors` (failed call → corrected call → confirmed success) is the only entry today. Default `[]` |
  | `capture.custom` | string | What is worth keeping, in your own words. Enables free-form observation capture through the built-in evaluator, scoped by this text. Mutually exclusive with `agent` |
  | `capture.agent` | string | Path to an `.agentuse` file that replaces the built-in evaluator, relative to this agent file, the same pattern as `verify.judge`. Its candidates are still vetted. Mutually exclusive with `custom` |
  | `apply` | boolean | Inject stored, vetted, active learnings into future runs. Default `true` |
  | `max` | number | How many learnings this agent keeps, and so how many may reach the model per run. 1-50, default 15. `agentuse doctor` shows what this costs and how many are stored but never injected |
  | `model` | string | Helper model used for capture, vetting, and `agentuse learnings tidy`. Defaults to the agent's own model |

  <Warning>
    `criteria` was removed in v0.18.0 and is a hard parse error:

    ```
    "criteria" was removed — free-form capture is opt-in now. To restore scoped free-form capture, move it to capture: { custom: "..." }
    ```

    Setting both `custom` and `agent` is likewise a parse error.
  </Warning>

  <Note>
    Boolean `capture: true` still parses, but its meaning narrowed in v0.18.0
    from free-form auto-capture to corrections only. An agent carrying it (or
    `learning: true`) warns once at parse time and runs the narrower behaviour;
    `agentuse doctor` echoes the notice.

    `file` and `evaluate` were removed in v0.17.0 and are rejected by name at
...
<ParamField path="reasoning" type='"none" | "minimal" | "low" | "medium" | "high" | "xhigh"' required={false}>
  Provider-agnostic reasoning effort, the recommended knob. AgentUse passes it to
  the AI SDK as the top-level `reasoning` level, which each provider maps to its
  own control: Anthropic to a thinking budget (a percentage of `maxOutputTokens`),
  OpenAI to `reasoningEffort`. One setting that works across Claude and OpenAI.
  ...
  Use `medium`/`high` for genuine judgment (hard calls under competing
  ...
  <Note>
    Prefer this over the provider-specific `openai.reasoningEffort` /
    `anthropic.thinking.budgetTokens` below, which are escape hatches for exact
    control and are honored only when the top-level `reasoning` is unset.
    Being top-level, it also avoids the "wrong-level key silently dropped" trap a
    misplaced `thinking:` hits.
  </Note>
</ParamField>

<ParamField path="openai" type="object" required={false}>
  OpenAI-specific options for GPT-5 and other OpenAI models. `reasoningEffort`
  here is the exact-control alternative to the top-level `reasoning` above.
...
<ParamField path="anthropic" type="object" required={false}>
  Anthropic-specific options for Claude models. `thinking.budgetTokens` is the
  exact-budget alternative to the top-level `reasoning` above; use it only
  when you need to pin an exact token budget.

  **Supported Options:**
  - `thinking.budgetTokens`: Enables Claude [extended thinking](https://platform.claude.com/docs/en/build-with-claude/extended-thinking) with the given token budget (minimum `1024`). When set, Claude streams its reasoning, which appears inline in the [session trace](/reference/session-storage). Honored only when the top-level `reasoning` is unset.
...
<ParamField path="skills" type='"auto" | "trusted" | string[] | object' required={false}>
  ...
  `auto` preserves the default behavior: all discovered skills are available for on-demand loading when relevant. A discovered skill is granted nothing until trusted (or its commands are listed in `tools.bash.commands`).

  **Trusting a skill grants it the bash commands it declares** in its SKILL.md `allowed-tools`. Trust per skill (recommended) or globally:

  ```yaml
  skills:
    linkedin: trusted     # grant just this skill's declared commands
  # or: skills: trusted    # trust every discovered skill (blunt shortcut)
  ```

  Trust only grants (the commands can run); gating is your explicit call. To require approval for a subset a skill grants (e.g. trust grants `birdc *` but you want `birdc reply *` approved), add that pattern to [`tools.bash.gated`](/guides/approval-gates#gated-commands-mechanical-enforcement); gated-wins precedence gates it while the rest of the family auto-runs. `agentuse doctor` flags granted commands that look irreversible.

  To preload a skill before the task starts without trusting it, define it explicitly (grant its commands yourself via `tools.bash.commands`):

  ```yaml
  skills: [linkedin]
  ```

  Inspect what trust granted with `agentuse doctor <agent-file>`.
  ...
  skills:
    auto: true
    linkedin:          # preload, but grant no commands
    x-personal: trusted
  ```

  Per-skill entries accept only the `trusted` grant. To grant a command without
  trusting the skill, list the command explicitly in `tools.bash.commands`.
  The removed `allow` key is invalid.
...
tools:
  ...
  bash:
    commands:                      # auto-run allowlist
      - "git *"
      - "npm *"
    gated:                         # run only after human approval (per-action lease)
      - "npm publish *"
  artifacts: true                  # enables artifact_save and artifact_list
  metrics: true                    # enables record_metric
```

`tools.bash.gated` lists commands that need human sign-off: they run only after an approval covering the exact action, and declaring `gated` enables the approval gate automatically. See [Gated commands](/guides/approval-gates#gated-commands-mechanical-enforcement).
...
mcpServers:
  filesystem:
    ...
    disallowedTools:      # Optional: tools to exclude
      - write_file
      - delete_file
    toolTimeout: 90       # Optional per-tool-call timeout: bare number = seconds
                          # (default 60), or a duration string like "2m".
                          # Env fallback: MCP_TOOL_TIMEOUT (seconds).
```
