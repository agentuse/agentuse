...
<ParamField path="skills" type='"auto" | "trusted" | string[] | object' required={false}>
  ...
  **Trusting a skill grants it the bash commands it declares** in its SKILL.md `allowed-tools`. Trust per skill (recommended) or globally:
  ...
  A trusted skill's irreversible commands are gated, not auto-run: the skill can mark them in SKILL.md (`metadata.agentuse-gated`), and AgentUse also auto-gates a granted command that looks irreversible (e.g. `deploy *`). A command you list yourself in `tools.bash.commands` is respected as-is and never auto-gated. See [Gated commands](/guides/approval-gates#gated-commands-mechanical-enforcement).
