...
<ParamField path="schedule" type="object" required={false}>
  Cron-based scheduling for automatic agent execution in serve mode.

  **Schedule Types (mutually exclusive):**
  - `cron`: Standard cron expression (e.g., `"0 * * * *"`)
  - `interval`: Simple interval format (`5s`, `10m`, `2h`, `1d`)
  - `every`: Natural language aliases (`"every 5 minutes"`, `"hourly"`, `"daily"`)

  **Additional Options:**
  - `enabled`: Enable/disable the schedule (default: `true`)
  - `timezone`: Timezone for schedule execution (default: system timezone)
  ...
  schedule:
    every: "hourly"        # Supported: every minute, every 5/10/15/30 minutes, hourly, daily, weekly, monthly
  ```
...
mcpServers:
  ...
  http_server:
    ...
    auth:
      ...
      token: ${env:API_TOKEN}
    requiredEnvVars:
      - API_TOKEN         # Validates API_TOKEN exists (used by ${env:API_TOKEN})
```
...
- **`auth`**: Authentication configuration object (optional)
  ...
  - **`token`**: Bearer token value, can reference environment variables with `${env:VAR_NAME}` syntax
