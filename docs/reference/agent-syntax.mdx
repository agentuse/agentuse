...
<ParamField path="reasoningEffort" type='"none" | "minimal" | "low" | "medium" | "high" | "xhigh"' required={false}>
  ...
  <Note>
    ...
    control and are honored only when the top-level `reasoningEffort` is unset.
...
<ParamField path="openai" type="object" required={false}>
  ...
  here is the exact-control alternative to the top-level `reasoningEffort` above.
