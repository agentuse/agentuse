...
<ParamField path="learning" type="boolean | object" required={false}>
  ...
  `capture.custom` or `capture.agent`.
  ...
  |-----|------|-------------|
  | `capture` | boolean \| object | Automatic observation configuration. `true` or `{}` enables no automatic channel; human learning uses Learn/`--remember`. Name `addons`, `custom`, or `agent` to opt in. Default `true` |
