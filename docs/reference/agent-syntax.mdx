...
<ParamField path="notifications" type="object" required={false}>
  Configure event/channel notifications separately from approval policy.
  ...
  **Fields:**
  - `routes`: ordered notification subscriptions.
  - `routes[].name`: optional route name for logs and debugging.
  - `routes[].enabled`: optional switch for temporarily disabling a route.
  - `routes[].on`: events for that route. Supported values are `approval`, `completion`, and `failure`. Use `completion`, not `complete` or `completed`.
  - `routes[].to`: one destination for that route. Use `slack`, `slack: {}`, or `slack.channel_id`. If omitted from Slack config, AgentUse uses `SLACK_APPROVAL_CHANNEL`.

  See [Notifications](/guides/notifications) for event semantics, Slack setup, and examples.
</ParamField>
