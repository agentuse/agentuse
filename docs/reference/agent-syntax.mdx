...
<ParamField path="openai" type="object" required={false}>
  ...
  **Supported Options:**
  ...
  - Defaults: when omitted, AgentUse does not set these fields and uses the OpenAI/AI SDK defaults. For reasoning effort, that is typically `medium` on reasoning models.
