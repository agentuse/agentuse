...
<ParamField path="model" type="string" required>
  ...
  **Named aliases.** `@name` resolves through the `models.aliases` block of your
  ...
  limited to transient or authentication failures before a fresh session
  receives any model output or tool call; resumes remain pinned to the concrete
...
<ParamField path="learning" type="boolean | object" required={false}>
  ...
  `learning: true` is shorthand for `{ capture: true, apply: true }`.

  ```yaml
  learning:
    capture: true
    apply: true
    criteria: Only keep instructions about tone and sourcing.
    max: 20
  ...
  |-----|------|-------------|
  | `capture` | boolean | Distil reviewer feedback into stored instructions. Default `true` |
  | `apply` | boolean | Inject stored instructions into future runs. Default `true` |
  | `criteria` | string | What is worth keeping, in your own words |
  | `max` | number | How many learnings may reach the model per run. 1-50, default 15. `agentuse doctor` shows what this costs and how many are stored but never injected |
  | `model` | string | Helper model used for capture and `agentuse learnings tidy`. Defaults to the agent's own model |

  <Note>
    `file` and `evaluate` were removed and are now rejected by name at parse
    time, e.g. `Unrecognized key(s) in object: 'file'`. Learnings live in the
    AgentUse state directory (`agentuse learnings migrate --all` moves older
