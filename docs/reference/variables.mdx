...
To show path-placeholder syntax literally in the model prompt instead of
resolving it, escape the dollar sign with a backslash: `\${root}` becomes the
literal text `${root}`. This is useful when an agent reviews or authors another
AgentUse file. The escape applies to `${root}`, `${agentDir}`, and `${tmpDir}`;
it does not enable environment-variable expansion.
