...
Installed global packages are recorded under `~/.local/share/agentuse/plugins`. Plugins installed with `agentuse plugins install -l` live under `.agentuse/packages` and are recorded in `.agentuse/plugins.json`.
