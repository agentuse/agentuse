...
├── learnings/        # agent corrections, one file per agent
└── consolidations/   # undo snapshots for `agentuse learnings tidy`
```

Corrections moved here in v0.17.0; they used to sit beside the agent file as `{agent-file}.learnings.md`, where every run rewrote them into your working tree. `agentuse learnings <agent-file> --path` prints the new location and `agentuse learnings migrate --all` moves existing files into it.
