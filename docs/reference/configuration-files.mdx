...
Common frontmatter fields include `model`, `tools`, `schedule`, `mcpServers`, `subagents`, `store`, `learning`, `sandbox`, `approval`, `notifications`, `timeout`, and `maxSteps`.

See [Agent Syntax](/reference/agent-syntax) for the full schema.

Approval notifications are configured separately from approval policy:
...
See [Notifications](/guides/notifications) for event route semantics and [Approval Gates](/guides/approval-gates) for the approval dashboard and API workflow.
