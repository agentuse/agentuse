...
- An object value defines ordered fallback `candidates`. On a transient first-request failure (rate limit, timeout, provider 5xx, or transport failure), AgentUse tries the next candidate only if the model has not produced output or invoked a tool.
- Optional `cooldown` is a duration string such as `"30s"` or `"5m"`. A transiently failing concrete model is skipped by new runs in the same process until the cooldown expires. Cooldowns are in memory: restarting `agentuse serve` or starting a separate CLI process resets them.

Aliases are resolved when the agent file is parsed. A fallback alias keeps its ordered concrete candidates until execution, then locks the session to the first model that responds successfully. Session logs, cost accounting, and context limits record that concrete model. A suspended run that resumes later stays on the model it started with, even if the alias or provider health has since changed.
...
| `$XDG_DATA_HOME/agentuse/` | Session logging, Learning | Per-project state, defaulting to `~/.local/share/agentuse/` |
