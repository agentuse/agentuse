...
<ParamField path="XDG_DATA_HOME" type="string">
  ...
  Everything a run generates is stored per project, under a directory keyed by a digest of the git root:

  ```
  $XDG_DATA_HOME/agentuse/project/{git-hash}/
  ├── session/          # execution logs
  ├── learnings/        # agent learnings
  └── consolidations/   # undo snapshots for `agentuse learnings tidy`
  ```

  Pointing this at a scratch directory therefore isolates a development daemon's sessions and learnings from your own.
