...
Large tool results are re-sent to the model on every subsequent step, so a single oversized output (a big diff, a verbose log, a huge file) inflates input tokens for the rest of the run. These variables cap how much of any one tool result reaches the model. Defaults match historical behavior, so you only need to set them to tune.

<ParamField path="AGENTUSE_TOOL_MAX_OUTPUT_BYTES" type="number">
  Maximum size (in characters) of a single `bash` tool result. Output over this limit is truncated to a head + tail slice, dropping the middle, with a marker noting how much was omitted.
  Default: 30720 (30KB)
