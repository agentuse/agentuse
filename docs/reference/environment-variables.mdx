...
<ParamField path="SLACK_BOT_TOKEN" type="string">
  Slack bot token used to post and update notification messages when `notifications.notify_on` includes `approval` and `notifications.channels.slack.enabled` is true. The app needs the `chat:write` bot scope, and the bot must be in the approval channel unless you also grant `chat:write.public` for public channels.
...
<ParamField path="SLACK_APPROVAL_CHANNEL" type="string">
  Default Slack channel id for approval notifications. Agents can override this with `notifications.channels.slack.channel_id`.
