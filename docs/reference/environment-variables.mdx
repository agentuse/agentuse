...
<ParamField path="SLACK_BOT_TOKEN" type="string">
  Slack bot token used to post and update notification messages when a notification route listens for `approval` and sends to Slack. The app needs the `chat:write` bot scope, and the bot must be in the approval channel unless you also grant `chat:write.public` for public channels.
...
<ParamField path="SLACK_APPROVAL_CHANNEL" type="string">
  Default Slack channel id for approval notifications. Agents can override this with `notifications.routes[].to.slack.channel_id`.
