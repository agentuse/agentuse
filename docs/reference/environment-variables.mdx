...
<ParamField path="AGENTUSE_MOCK_MODE" type="boolean">
  `1` to mock all tool outputs (no real bash/filesystem/MCP/store side effects). Same as `--mock`.
</ParamField>

<ParamField path="AGENTUSE_MOCK_MODEL" type="string">
  Model used for mock outputs. Defaults to the agent's model; use `demo:default` for zero cost. Same as `--mock-model`.
</ParamField>
