...
```yaml
---
model: anthropic:claude-sonnet-4-6:dev  # Uses ANTHROPIC_API_KEY_DEV
---
```
...
<ParamField path="SLACK_BOT_TOKEN" type="string">
  Slack bot token used to post and update approval messages when `approval.channel` is `slack`. The app needs the `chat:write` bot scope, and the bot must be in the approval channel unless you also grant `chat:write.public` for public channels.
...
<ParamField path="SLACK_APP_TOKEN" type="string">
  Optional Slack app-level token used by the legacy Socket Mode approval listener. Web approval pages do not require this token. If set, the app-level token needs `connections:write`.
...
<ParamField path="SLACK_APPROVAL_CHANNEL" type="string">
  Default Slack channel id for approval messages. Agents can override this with `approval.channel_id`.
...
<ParamField path="APPROVAL_WEBHOOK_URL" type="string">
  Conventional environment variable for webhook approval notifications when using `approval.url: ${env:APPROVAL_WEBHOOK_URL}`.
...
<ParamField path="AGENTUSE_RESUME_PUBLIC_URL" type="string">
  Fallback public base URL used to build webhook resume links. Prefer `agentuse serve --public-url ...` or `serve.publicUrl` for hosted deployments.
